# Normverificatie — Dossier Elektriciteit

Gestart 04-07-2026. Protocol: elke harde claim krijgt status `[GEVERIFIEERD: bron(nen), datum]` of `[NIET GEVERIFIEERD]`; getallen/drempels/verplichtingen nooit op één bron; primaire bronnen (AFNOR, Légifrance, Consuel, Enedis, service-public.fr) leidend, serieuze fabrikanten (Hager, Schneider, Legrand) en Promotelec als convergentiecheck — zij hebben eigen aansprakelijkheid en normcommissie-zetels, maar blijven secundair.

---

## V1 — Status NF C 15-100 · [GEVERIFIEERD: AFNOR (norm-nieuwspagina + boutique), Consuel-actualités, Hager, Schneider, Socotec — 04-07-2026]

**Vastgesteld, convergent over zes bronnen:**
1. De NF C 15-100 is in **augustus 2024** door AFNOR gepubliceerd als **volledige herschrijving** ("refonte globale/totale") — géén zesde amendement. Het monolithische document uit 2002 (+ update 2005, + amendementen A1–A5 2008–2015, + interpretatiefiches F11–F27) is vervangen door een **serie van 21 zelfstandige normdelen** (o.a. deel 1 algemene eisen; 7-701 badkamers; 7-702 zwembaden; **7-722 laadinfrastructuur/IRVE**; **8-1 energie-efficiëntie**; **deel 10 woningen**; deel 11 communicatienetwerken). Reden van de opdeling: delen kunnen voortaan afzonderlijk worden herzien.
2. **Overgangsjaar**: vanaf publicatie één jaar vrijwillige toepassing; **systematische toepassing sinds eind augustus / 1 september 2025**. De serie is **niet retroactief**: zij geldt voor nieuwe installaties, renovaties en de gewijzigde delen van bestaande installaties.
3. **Peildatum** voor welke normversie geldt: datum indiening permis de construire, anders déclaration préalable, anders ondertekening van de aannemingsovereenkomst (Consuel + AFNOR). In 2026 geldt voor elk nieuw project de 2024-serie.
4. **Regelgevingsnuance (AFNOR zelf, juni 2026):** de 2024-serie is nog **niet in de regelgeving verankerd** als verplichte norm; formeel blijft de 2002-versie c.s. de "norme rendue obligatoire" in de zin van het **arrêté van 3 augustus 2016**, en die oudere versie blijft daarom gratis raadpleegbaar. Praktisch (Consuel-keuring, aannemers, fabrikanten) is de 2024-serie sinds september 2025 de werkstandaard. → In H8 uitleggen als "norm vs. verplichtstelling", niet wegmoffelen.
5. **Inhoudelijke kernwijzigingen** (Hager/Schneider/Socotec/kasq, convergent): nieuw **type F-aardlekbeveiliging** voor circuits met frequentieregelaars (warmtepomp, airco, zwembadpomp, laadpunt); introductie **vlamboogbeveiliging (AFDD/protecteur d'arc)**; eigen normdeel IRVE (7-722); energie-efficiëntie als aanbevelingenkader (8-1, o.a. barycentre-methode); **deel 10 (woningen) inhoudelijk weinig gewijzigd t.o.v. Amendement 5** — Robs woninghoofdstukken blijven dus in de kern bruikbaar, met bovenstaande aanvullingen.

**Genoteerde brononenigheden (klein, wel vastleggen):**
- Publicatiedatum: 21 aug (kasq) vs. 23 aug 2024 (Socotec, Hager). AFNOR zelf houdt het op "augustus 2024". → In het dossier: "augustus 2024", geen dagprecisie claimen.
- Aantal delen: AFNOR-boutique zegt "serie van 20 normen", AFNOR-nieuwsbericht en alle overige bronnen "21". → aanhouden: 21, met AFNOR-nieuwsbericht als bron.
- Verplichte-toepassingsdatum: "24 augustus 2025" (Hager) vs. "1 september 2025" (Socotec) vs. "september 2025" (Consuel). → In het dossier: "sinds september 2025", geen dagprecisie.

**Consequenties voor het dossier:**
- H8 wordt geschreven rond de 2024-serie + de regelgevingsnuance; de normgeschiedenis (2002 → A5 2015 → refonte 2024) als kort kader, met Robs Sinterklaas- en gloeilampanekdotes als kleur.
- H3 (groepenkast): type F en AFDD opnemen; alle ID-regels toetsen aan NF C 15-100-1 en deel 10, niet aan A5.
- H7: NF C 15-100-7-722 is de normbasis voor het laadpunt-deel; IRVE-installateurskwalificatie apart verifiëren (V7).
- Robs conserveerbare woningmateriaal: legitiem, want deel 10 is grotendeels continu met A5 — maar élke overgenomen regel wordt individueel getoetst.

---

**Aanvulling V1 (04-07-2026, bron aangedragen door Anton — vaste ankerbron):** AFNOR-nieuwspagina *"Installations électriques à basse tension : la série NF C15-100 est parue"* (gepubliceerd 13-09-2024, laatst bijgewerkt 04-06-2026) — dit wordt DE ankerbron voor de normstatus in H8:
- Bevat de **volledige lijst van alle 21 normdelen met directe AFNOR-boutique-links** → overnemen in het bronnenregister van het dossier.
- Citaat commissielid Bernard Gioan (FFIE, commissie U15): *"Le titre 10 de 2016, qui concerne les habitations, reste inchangé"* en NF C15-100-1 bundelt de oude titels 1–6. **Deel 10 (woningen) is dus niet "weinig gewijzigd" maar formeel ongewijzigd gebleven (2016/A5-tekst)** — sterkere legitimatie voor het conserveren van Robs woningmateriaal; de vernieuwing zit in deel 1, 7-722, 8-1 en 11.
- 7-722 (EV): circuit spécialisé verplicht; **obturateurs blijven vereist bij normaal laden**; nieuwe rekenregels kabelsecties i.v.m. harmonischen (frequentieregelaars).
- Brandveiligheid: euroclasses (CPR) voor geïsoleerde geleiders/kabels verankerd; gewijzigde parafoudre-risicomethode geldt *"à l'exception des bâtiments d'habitation"* — relevant voor V6: voor wóningen blijft dus de AQ-systematiek het kader.
- Regelgevingsnuance letterlijk bevestigd (versie 2002 + amendementen blijft "norme d'application obligatoire" o.g.v. arrêté 03-08-2016 en gratis raadpleegbaar).
- Boekentip voor H9: AFNOR Éditions, *"Installation électrique des logements : la NF C15-100 simplement"* (maart 2026) — de natuurlijke opvolger van de door Rob betreurde Promotelec-boekjes.

## V9 — Diagnostic électrique bij verkoop/verhuur · [GEVERIFIEERD: service-public.gouv.fr F18692 (primair) + Qualitel, Engie, Empruntis convergent — 04-07-2026]

1. **Verplicht** bij verkoop én verhuur van een woning waarvan de elektrische installatie **ouder is dan 15 jaar** (incl. dependances met vaste installatie). Verkoop sinds 2009, verhuur sinds 2017/2018. Onderdeel van het Dossier de Diagnostic Technique (DDT), vóór compromis/bail.
2. **Geldigheid: 3 jaar bij verkoop, 6 jaar bij verhuur.** Een **Consuel-attestation jonger dan 6 jaar vervangt het diagnostic** — direct relevant voor wie recent renoveerde.
3. **Karakter**: veiligheidsonderzoek (aarding, differentieelbeveiliging, verouderd materieel, badkamerregels), **géén conformiteitstoets aan de actuele NF C 15-100**. Cruciale nuance voor het dossier: "niet conform de nieuwste norm" is op zich geen afkeurgrond; gevaarlijke anomalieën zijn dat wel voor de prijsvorming.
4. **Consequenties**: herstel is niet wettelijk verplicht, maar (a) zonder diagnostic of bij misleidende informatie geen vrijwaring van verborgen gebreken — koper kan via de rechter ontbinding of prijsvermindering eisen, notaris kan aansprakelijk zijn; (b) elke gerapporteerde anomalie is een onderhandelingsargument voor de koper; (c) bij verhuur kan een ernstige anomalie de verhuurbaarheid (logement décent) blokkeren en ligt aansprakelijkheid bij schade bij de eigenaar.
5. Wettelijke basis: Code de la construction et de l'habitation art. L134-6/L134-7 (service-public); uitvoering arrêté 28-09-2017; diagnostiqueur moet gecertificeerd (Cofrac-geaccrediteerd) en verzekerd zijn.
6. Bijvangst V12: het brandcijfer "250.000 woningbranden / 80.000 elektra-oorzaak" uit Robs intro circuleert nog steeds in secundaire bronnen (Empruntis via Figaro Santé), plus ONSE-cijfers (7 mln woningen met gevaarlijke installatie; 68% van >15 jaar met minstens één anomalie). → primaire ONSE-bron nog raadplegen vóór publicatie (V12 blijft open).

---

## V2 — Aardlek-/ID-regels woningen · [GEVERIFIEERD: Legrand (pro + 2× particulier), 123elec, Cedeo, Samse, IZI by EDF — convergent, 04-07-2026]

1. **Alle circuits** onder DDR ≤ 30 mA aan de oorsprong. **Minimaal 2 interrupteurs différentiels per woning**, verlichting- en stopcontactcircuits verdeeld over **minstens 2 ID's** (continuïteit bij uitval).
2. **Maximaal 8 circuits per ID**, ongeacht kaliber — bevestigt Robs weergave; de oude oppervlakteregels zijn sinds A5 definitief vervangen door circuit-gebaseerde logica (continuïteit in de 2024-serie).
3. **Typen**: minimaal 1 **type A** (kookplaat, wasmachine, EV-laadpunt mono); overige circuits type AC of A. **Nieuw en verplicht sinds de 2024-serie: type F** aan het hoofd van circuits met éénfasige frequentieregelaar (warmtepomp, airco, zwembadpomp); zelfde toepassingen in driefase: **type B**. EV-laden: type A of F (mono), type B (tri) — 123elec o.b.v. revisie 2024.
4. **Kaliber (40/63 A)**: amont-regel (≥ kaliber disjoncteur de branchement) óf aval-regel (≥ 1× som beveiligingen verwarming + warmwater + EV-laadpunt, + 0,5× som overige). Let op: Legrand-pro meldt dat de amont-regel in **NF C 15-100-10, tabel 10-1G** is aangepast voor aanvullende energiebronnen (zonnepanelen/batterij) — detail voor H3 opzoeken in Consuel/fabrikantengids bij het schrijven. Robs praktijkopmerking "tegenwoordig vaak 63 A" is consistent hiermee.
5. **Reserveruimte**: 20% vrije plaats, minimaal 6 modules (sinds A5; niet weersproken voor 2024-serie).
6. AFDD/DPDA (vlamboogbeveiliging): in de 2024-serie **aanbevolen**, niet verplicht (Socotec, One-Elec); type F is wél een verplichting — dit onderscheid expliciet zo in H3.
7. **Verworpen uitschieter**: één bron (One-Elec) noemt "één ID per 5 circuits" — strijdig met alle overige bronnen (8), niet overnemen. Bijvangst voor V6: parafoudre-afstandsregel zou in de 2024-serie zijn aangescherpt van 30 m naar 10 m (One-Elec) — [INDICATIE, apart verifiëren bij Consuel/Promotelec].

---

## V3 — Aarding · [DUBBEL ONAFHANKELIJK GEVERIFIEERD (twee sessies, 04-07-2026): schema-electrique.net (2024-editie, 2×), Socotec, Legrand, Assurly, materiels-electriques, egpp]
1. Aardingsweerstand: **< 100 Ω wettelijk maximum**, zo laag mogelijk; advies **< 50 Ω**, bij parafoudre strenger — bronnen adviseren tot **< 10 Ω**. Robs forumwaarde "50 Ω bij parafoudre" valt binnen de adviesband; in H4 de band 50→10 benoemen.
2. **Robs 2-Ohm-regel bevestigd, met precisering**: ≤ 2 Ω geldt voor de continuïteit van beschermingsleidingen en equipotentiaalverbindingen (ohmmeter-meting), incl. de vrijstellingsregel (douchebak/badkuip/kozijnen niet aansluiten bij continuïteit ≤ 2 Ω of isolatie ≥ 500 kΩ). Niet formuleren als "stopcontact naar aardstrip" sec.
3. Boucle à fond de fouille verkieslijk boven enkele aardpen; waterleidingen/putten als aardelektrode verboden (wel aan LEP koppelen). Beschermingsleiding altijd groen-geel; nul (licht)blauw; functionele aarde in andere kleur — basis voor V13-kleurentabel.
4. LES/LEL: verplicht in bad-/doucheruimten; min. 2,5 mm² koper; sinds A5 interconnectie bij het tableau toegestaan; drie toegestane uitvoeringen; verbinding naar hoofdaardklem min. 6 mm².

## V4 — Badkamerzones (7-701) · [DUBBEL GEVERIFIEERD: Promotelec, Legrand (pro + particulier), Socotec, schema-electrique, Espace Aubade — 04-07-2026]
1. **Promotelec expliciet: de volumeregels (0/1/2 + hors volume) zijn in de 2024-serie ongewijzigd** (volume 3 al sinds A5 vervallen); zwembadvolumes ongewijzigd, wel uitbreiding naar natuurwater. **De lijst te verbinden LES-elementen is aangepast** — exact detail opzoeken bij het schrijven van H6.
2. Volume caché onder bad/douchebak: geen apparatuur (IP44-uitzondering achter 30 mA). IP-ladder: volume 0 alleen TBTS IPX7; volume 1 IPX5; volume 2 IPX4 (scheerstopcontact TBTS toegestaan); hors volume vrij. Verlichting volume 2: DCL met IPX4. Stopcontacten alleen hors volume. Bedieningshoogtes 0,90–1,30 m.
3. Alle badkamercircuits achter 30 mA-ID + LES. Robs badkamermateriaal blijft grotendeels geldig; UK-doorstroomverwarmers passen niet in de volumesystematiek (H6-kader).

## V5 — Circuits, stopcontacten, telregels · [DUBBEL GEVERIFIEERD: normtekst 10.1.3.3 (via forumcitaat), schema-electrique (3 p.), Legrand, Schneider, IZI by EDF, kasq, 123elec, Promotelec — 04-07-2026]
1. **Telregel per socle** (dubbel blok = 2, driedubbel = 3); de soepelere oude telling is per A5 geschrapt en niet teruggekeerd. ⚠️ **Oogst-item 30 (Robs teltip) definitief VEROUDERD** — alleen als historische voetnoot met correctie.
2. Circuits prises: max. 8 socles op 1,5 mm²/16 A of 12 op 2,5 mm²/20 A. Verlichting: min. 2 circuits (studio 1), max. 8 punten/circuit (prise commandée telt mee), DCL verplicht (plafonddoos 25 kg).
3. Keuken ≥ 4 m²: 6 niet-gespecialiseerde prises op eigen circuit 2,5 mm²/20 A (max. 6 op dat circuit; extra via ander circuit), 4 boven werkblad op 8–25 cm; < 4 m²: 3. Verboden boven évier/kookplaat; hotte-prise ≥ 1,80 m, gemarkeerd.
4. **Minimaal 4 circuits spécialisés**: kookplaat 32 A/6 mm² + ≥ 3× 20 A/2,5 mm² (oven, vaatwasser, wasmachine — wasmachine onder type A); studio-uitzondering 3 (bronvariant "32 A + 2" genoteerd). Indien aanwezig elk eigen circuit: droger, chauffe-eau, verwarming (per 4.500 W), warmtepomp/airco, VMC, **gemotoriseerde rolluiken (2024: eigen circuit)**, EV-laadpunt; koelkast/vriezer aanbevolen.
5. Minima per vertrek (deel 10 ≈ A5): séjour 1 per 4 m², min. 5, plafond 7 (> 28 m²), open keuken −8 m² forfait; slaapkamer 3; elk vertrek/gang > 4 m²: 1; prise naast RJ45/tv-punt én naast de lichtschakelaar bij de entree; hoogtes as ≤ 1,30 m (advies > 0,40 m), 16 A ≥ 5 cm, 32 A ≥ 12 cm; obturateurs t/m 32 A; klauwbevestiging verboden.
6. RJ45/communicatie: verplichte uitrusting (arrêté 03-08-2016): 2 accolées in séjour + uitbreiding naar aantal kamers (wachtkabel-alternatief) — bevestigt oogst-item 44.
7. Bijvangst H7: collectief woongebouw met afgesloten parkeren → minimaal één EV-laadvoorziening verplicht (2,5 mm²/20 A + passende differentieel).

## V6 — Parafoudre · [DEELS GEVERIFIEERD: Legrand pro/gids 2024 — 04-07-2026]
1. **Verplicht in AQ2-gebieden** (> 25 onweersdagen/jaar, departementenlijst) conform NF C 15-100-1/-10; **aanbevolen** < 50 m van gebouw met paratonnerre met gevoelige apparatuur.
2. [OPEN] Afstandsregel: één bron meldt aanscherping 30 m → 10 m in de 2024-serie; strijdig met Legrands 50 m-formulering → beslechten bij Consuel/Promotelec. Robs detailregels (parafoudre-aarddraad max. 50 cm; auto-protégé vs. eigen disjoncteur) apart toetsen bij H3. Aarde-advies < 10 Ω: zie V3.

## V7 — EV-laadpunt / IRVE · [GEVERIFIEERD: décret 2017-26 (uitzonderingsbepaling), Engie, Qualit'EnR, Ulys, meerdere IRVE-bronnen — 04-07-2026]
1. **Installatie > 3,7 kW uitsluitend door IRVE-gekwalificeerde installateur** (décret n° 2017-26; kwalificatie Qualifelec/AFNOR, niveaus 1–3). Uitzondering ≤ 3,7 kW in niet-publiek toegankelijke privéwoning — maar ook een prise renforcée vergt eigen circuit + 30 mA type A, en zonder IRVE-factuur geen TVA 5,5% en risico op verzekeringsweigering.
2. Normbasis **NF C 15-100-7-722** (vervangt UTE C 15-722): eigen circuit; differentieel mono type A of F; **tri: type B**.
3. **Subsidies: het crédit d'impôt (500 €) is per 31-12-2025 gestopt, niet verlengd in 2026** (declaratie 2026 alleen voor 2025-betalingen). Resteert: TVA 5,5% (alleen via IRVE-installateur) en prime Advenir (copropriétés). → tijdgevoelig blok in H7 met verificatiedatum.
4. Kabeldoorsnedes uit de praktijk (10 mm² e.d.) zijn installateursdimensionering, geen normclaim — indicatief labelen.

## V8 — Consuel: attestaties, tarieven, zelfaanleg · [DUBBEL GEVERIFIEERD: consuel.com (barème arrêté 04-08-2015, +0,64% per 02-09-2025) + zes secundaire bronnen — 04-07-2026]
1. Vier attestaties: **geel** 12506*03 (woningen, incl. IRVE in habitat), **groen** 12507*03 (niet-domestiek/gemeenschappelijk), **blauw** 15523*01 (productie zonder opslag), **violet** 15524*01 (met opslag). Woning + zonnepanelen = geel + blauw/violet (twee formulieren).
2. Particulierentarieven metropool, elektronisch (per 02-09-2025): **geel 144,67 €** (papier 146,15), groen 76,37, blauw 201,17, violet 230,32; **contre-visite 232,13 €**. Barème bij arrêté, landelijk uniform; bij publicatie exact aflezen van consuel.com. Verworpen als verouderd: EDF-pagina "~125 €".
3. **Zelfaanleg toegestaan en voorzien**: auteur van de werken (particulier bij auto-construction) vraagt aan en tekent; particulieren krijgen vrijwel altijd controlebezoek, bekende pro's steekproefsgewijs. Aanvraag ±20 dagen vooraf; blanco formulier 12 maanden geldig; sinds décret 2024-1122 gaat de gevisieerde attestatie automatisch naar Enedis; vanaf april 2026 staat NF C 15-100:2024 standaard aangevinkt in Mon Espace Consuel. Zonder visa geen inbedrijfstelling.
4. Blauw/violet-dossier: onduleur-certificaat NF EN 50549-1/-2, sinds 01-01-2026 ook -10:2023 (≤ 36 kVA) — raakvlak H7/H8.
5. Kadermateriaal H8: Consuel opgericht 1964, verplicht per decreet 14-12-1972 na de brand in dancing "5-7" (146 doden) — historische duiding bij Robs "waarom zo streng"-lijn.


## V11 — Tariefopties & de heures-creuses-hervorming · [GEVERIFIEERD: Enedis (2 officiële pagina's), service-public.gouv.fr A18476, EDF, CRE-deliberatie 08-01-2026 via convergente bronnen — 04-07-2026]
1. **De grootste HP/HC-hervorming sinds de invoering** (CRE/TURPE 7, maart 2025): vanaf **01-11-2025** verschuift Enedis de daluren van 14,5 mln HP/HC-klanten (11 mln geraakt; 3,5 mln al conform). **Fase 1** nov 2025–jun 2026: 1,7 mln klanten met bestaande dag+nacht-plages; **fase 2** dec 2026–okt 2027: 9,3 mln + seizoensdifferentiatie; professionals sluiten 2027 af.
2. Nieuwe systematiek: nog steeds 8 daluren/dag, maar **minimaal 5 aaneengesloten 's nachts (23h–7h)** + **maximaal 3 overdag (11h–17h)**; zomer (1/4–31/10) mét middagvenster, winter vooral nachts. Oude vensters 7h–11h en 17h–23h verdwijnen. Enedis wijst de plages toe per aansluitpunt (geen keuze); leverancier informeert ≥ 1 maand vooraf; Linky en de contacteur jour/nuit (contact sec) schakelen automatisch mee — Robs PULSADIS-kader (oogst 54) krijgt hier zijn actuele vervolg.
3. **Tempo blijft ongewijzigd** (22h–6h landelijk). CRE-deliberatie 08-01-2026: wintervenster 11h–14h weer toegestaan voor nieuwe klanten. [TWEEDE BRON ZOEKEN vóór publicatie]: de melding dat de **Base-optie per 01-02-2025 is afgeschaft voor ≥ 9 kVA** (één bron) — indien waar, groot punt voor H1/H7.
4. Voor H1/H7: de oude vuistregel "50–60% verbruik in HC nodig" (oogst 53) moet ná deze hervorming opnieuw doorgerekend worden — geen oude drempels citeren zonder herberekening; de hervorming zelf wordt een tijdgevoelig blok met verificatiedatum.

## V13 — Draadkleuren NL/BE/FR · [GEVERIFIEERD (NL+BE+geharmoniseerd): NEN 1010-bronnen (3×), AREI-bronnen (elektricien.vlaanderen, BouwInfo, Wikipedia/FOD-verwijzing) — 04-07-2026; FR-historisch nog open]
1. **Modern identiek in alle drie de landen** (IEC/HD-harmonisatie, NL sinds 2001 in NEN 1010; BE via AREI/NBN EN 60446): fase bruin, nul blauw, aarde groen-geel (exclusief!). De vergelijkingstabel draait dus om de **historische lagen** die de klusser in oude huizen aantreft.
2. **NL-oud (< ±1970)**: fase **groen** (of zwart), nul **rood**, aarde grijs/blank, schakeldraad zwart — de dodelijke valkuil: oud-NL groen = fase, terwijl groen(-geel) nu aarde is. Mede gewijzigd vanwege rood-groen-kleurenblindheid. Dit wordt hét waarschuwingskader in H5.
3. **BE/AREI**: alleen groen-geel (aarde) is dwingend; blauw voor nul is vaste praktijk en verplicht waar een echte nul bestaat; fasekleur vrij. Belgische eigenaardigheid voor het blok: oudere 3×230V-netten zonder nul — twee fasen het huis in, "nul" bestaat daar niet. AREI volledig herzien per **01-06-2020** (3 boeken, FOD Economie) — ankerbron voor alle BE-claims.
4. **FR-historisch**: nog niet met onafhankelijke bron vastgesteld → [OPEN]; bij het schrijven van H5 toetsen (schema-electrique/Promotelec) en Robs eigen kleurenbeschrijving uit de-leidingaanleg daartegen leggen.

## V14 (deelresultaat) — Zelf aanleggen NL/BE
- **BE**: zelf aanleggen toegestaan, maar na nieuwe aanleg of belangrijke wijziging is **keuring door een erkend controleorganisme verplicht** (AREI Boek 1; gelijkvormigheidscontrole). Dit is het scherpste contrast met NL én met FR (waar Consuel alleen bij nieuwe aansluiting/herinschakeling speelt) — kernregel voor het "mag ik dit zelf?"-blok in H0.
- **NL**: geen wettelijk installateursmonopolie voor de eigen woning; NEN 1010 geldt via het Bouwbesluit/Bbl; geen verplichte particuliere keuring (wel verzekerings- en aansprakelijkheidsrisico). [Formele bronverwijzing Bbl-artikel nog toevoegen vóór publicatie.]
- FR-deel: gedekt in V8.

---

## Openstaande verificatiepunten
- V6 afronden: afstandsregel 50 m vs 10 m beslechten (Consuel/Promotelec) + Robs 50 cm-aarddraadregel
- V10 Linky: uitrolstatus, functies, weigeraars-regeling
- V11-rest: puissance-reeksen mono/tri actueel + kosten ombouw tri↔mono (Enedis Catalogue des Prestations) + Base-afschaffing ≥9 kVA tweede bron
- V12 Brandcijfer elektra-oorzaak (ONSE/Promotelec) voor H0
- V13-rest: FR-historische kleuren + draaddiktentabel NL/BE/FR
- V14-rest: NL-bronverwijzing Bbl; BE-keuringsdetails (termijnen, kosten indicatief)

## Werkafspraak (n.a.v. dubbelloop 04-07-2026)
Op 04-07 hebben twee Claude-instanties onafhankelijk aan dit bestand gewerkt (commits fc8d688/eee6e98 om 08:06 en 06260c1/a4120af om 08:41), met dubbele V-blokken als gevolg; per deze commit samengevoegd. De inhoud bleek convergent — feitelijk een onbedoelde dubbelcheck — maar dit mag niet nog eens: **één schrijver per bestand per dagdeel; de claude.ai-sessie doet de verificatie (V-punten), Claude Code raakt /verificatie/ niet aan tot de bouwprompt dat expliciet regelt.**

